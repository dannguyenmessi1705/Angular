...
export class LoginComponent {
  form = new FormGroup({
    ...
    password: new FormControl("", {
      ...
      // có thể truyền chỉ định trường vào validators ví dụ mustContainQuestionMark("password"), mặc định không truyền thì sẽ kiểm tra trường hiện tại được đưa vào.
    }),
  });
...
### 4.5. Lồng các FormGroup
Trong 1 form, có thể có nhiều trường, và bạn có thể nhóm chúng lại với nhau bằng cách sử dụng `FormGroup`. Ví dụ, trong form đăng ký, bạn có thể nhóm các trường liên quan đến địa chỉ lại với nhau trong một `FormGroup`.
Ngoài ra nhóm `FormGroup` đó có thể thêm validate chung cho cả nhóm kết hợp với các validate riêng của từng trường.
```ts
import { Component } from "@angular/core";
import {
  AbstractControl,
...
```
> Đối với template, bạn cần 1 thẻ tag để bọc các trường trong `FormGroup` lại với nhau. Với các thẻ không phải là `form`, bạn dùng `formGroupName="<tên nhóm>"` hoặc `[formGroup]="form.controls.<tên nhóm>"` để chỉ định nhóm cho các trường.
> Với template có form tùy chọn nhiều trường, bạn có thể sử dụng `formArrayName="<tên mảng>"` để chỉ định mảng cho các trường, sau đó trong từng trường bạn dùng `formControlName="<index></index>"` tương ứng với index vị trí các trường trong mảng khai báo `new FormArray()`.
Ví dụ:
```html
<form [formGroup]="form" (ngSubmit)="onSubmit()">
