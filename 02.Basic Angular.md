...
- **Cách sử dụng Signal**:
  ...
  - **Cập nhật Signal dựa trên giá trị trước đó**: Sử dụng cú pháp `signalName.update((prev) => newValue)` để cập nhật giá trị của signal dựa trên giá trị trước đó.
...
> Ngoài ra để tạo liên kết 2 chiều bằng cách sử sụng `ngModel` chúng ta cũng có thể sử dụng signal `model`. VD:
```ts
...
export class ExampleComponent {
  @model name: string = ''; // Biến để lưu tên người dùng
...
- Zone.js là một thư viện giúp theo dõi và quản lý các thay đổi trong ứng dụng Angular. Nó cho phép Angular phát hiện khi nào có sự thay đổi trong ứng dụng và tự động cập nhật giao diện người dùng mà không cần phải can thiệp thủ công.
- Zone.js tạo ra một "zone" (vùng) xung quanh mã của bạn, và bất kỳ thay đổi nào xảy ra trong vùng này đều được theo dõi. Khi một sự kiện xảy ra (như click chuột, nhập liệu, v.v.), Zone.js sẽ tự động kích hoạt quá trình kiểm tra và cập nhật giao diện người dùng.
- Điều này giúp giảm thiểu việc phải gọi `ChangeDetectorRef.detectChanges()` một cách thủ công và giúp mã của bạn trở nên sạch sẽ và dễ bảo trì hơn.
- Tuy nhiên, trong một số trường hợp, bạn có thể muốn chạy mã bên ngoài zone của Angular để cải thiện hiệu suất hoặc tránh kích hoạt change detection không cần thiết. (Zone Pollution)
...
export class ExampleComponent {
  ...
  runOutsideZone() {
    this.ngZone.runOutsideAngular(() => {
      // Mã này sẽ không kích hoạt change detection
    });
  }
}
```
### 10.1. Chiến lược `onPush`

- Chiến lược `onPush` là một trong những chiến lược change detection trong Angular. Khi sử dụng chiến lược này, Angular sẽ chỉ kiểm tra và cập nhật giao diện người dùng khi có sự thay đổi rõ ràng trong dữ liệu đầu vào của component và các component con của nó (trừ khi các component con này cũng sử dụng chiến lược `onPush`).
- Điều này giúp cải thiện hiệu suất của ứng dụng, đặc biệt là khi làm việc với các component phức tạp hoặc có nhiều dữ liệu.
- Để sử dụng chiến lược `onPush`, bạn cần thêm `ChangeDetectionStrategy.OnPush` vào decorator `@Component` của component.
...
### 10.2. Sử dụng `ChangeDetectorRef` và `BehaviorSubject (RxJS)`
- Trong Angular, bạn có thể sử dụng `ChangeDetectorRef` để kiểm soát quá trình change detection một cách thủ công. Điều này hữu ích khi bạn cần cập nhật giao diện người dùng mà không có sự thay đổi rõ ràng trong dữ liệu đầu vào.
- `BehaviorSubject` là một loại subject trong RxJS cho phép bạn phát đi giá trị mới và theo dõi các giá trị trước đó. Khi kết hợp `BehaviorSubject` với `ChangeDetectorRef`, bạn có thể dễ dàng cập nhật giao diện người dùng khi có sự thay đổi trong dữ liệu.
...
export class MessageService {
  private message$: BehaviorSubject<string[]> = new BehaviorSubject<string[]>([]); // BehaviorSubject<type>(init value), quy ước đặt tên của biến RxJS là sẽ có dấu `$` ở sau biến.
  ...
  addMessage(message: string) {
    ...
    this.message$.next(this.messages); // Cập nhật giá trị mới cho BehaviorSubject (đẩy dữ liệu cho các subscriber)
  }
}
```

```ts
import { Component, ChangeDetectorRef, OnInit, inject, DestroyRef } from '@angular/core';
...
export class ExampleComponent implement OnInit {
  private messageService = inject(MessageService); // Tiêm service vào component
  private cdRef = inject(ChangeDetectorRef); // Tiêm ChangeDetectorRef vào component
  private destroyRef = inject(DestroyRef); // Tiêm DestroyRef vào component

  messages: string[] = [];

  ngOnInit() {
    const subscription = this.messageService.message$.subscribe(messages => {
      this.messages = messages; // Cập nhật danh sách tin nhắn (messages được lấy từ BehaviorSubject vừa phát ra)
      this.cdRef.markForCheck(); // Đánh dấu component để kiểm tra lại
    });

    this.destroyRef.onDestroy(() => {
      subscription.unsubscribe(); // Hủy đăng ký khi component bị hủy để tránh rò rỉ bộ nhớ
    });
  }
}
```
...
> Cách khác nhanh hơn, chỉ cần sử dụng `BehaviorSubject` và kết hợp với `async pipe` trong template
```ts
...
### 10.3. Zoneless
- Zoneless là một khái niệm trong Angular cho phép bạn tắt chế độ zone.js cho một phần của ứng dụng. Điều này có thể cải thiện hiệu suất bằng cách giảm số lần kiểm tra thay đổi không cần thiết.
- Trên thực tế, việc sử dụng `signal` trong Angular có thể giúp bạn đạt được hiệu ứng tương tự mà không cần phải tắt hoàn toàn zone.js. `signal` cho phép bạn tạo ra các tín hiệu để thông báo cho Angular biết khi nào cần kiểm tra lại giao diện người dùng.
- Để tắt zone.js cho toàn bộ ứng dụng, thực hiện bước sau:
  - B1. Mở file `angular.json` và xóa `"zone.js"` trong thuộc tính `polyfills`.
  - B2. Trong file `main.ts` thêm tùy chọn providers `provideZonelessChangeDetection` vào config.
  ```ts
  import { bootstrapApplication } from '@angular/platform-browser';
...
## 11. Template control flow hiện đại
- **`@switch` / `@case` / `@default`**: Thay thế cho `ngSwitch`. Ưu tiên sử dụng cú pháp mới vì tree-shaking tốt hơn và giúp template dễ đọc hơn.
```html
@switch (status()) {
  @case ('pending')    { <p>Đang xử lý...</p> }
  @case ('success')    { <app-success [data]="result()" /> }
  @case ('error')      { <app-error [message]="errorMessage()" /> }
  @default             { <p>Không rõ trạng thái.</p> }
}
```
- **`@for` với `track`**: Nếu danh sách lớn hoặc được cập nhật thường xuyên, bắt buộc truyền `track` để Angular nhận diện phần tử nào cần cập nhật.
```html
@for (item of todos(); track item.id) {
  <app-todo-row [todo]="item" />
} @empty {
  <p>Không có dữ liệu.</p>
}
```
- **`@defer` + `@placeholder` + `@error`**: Trì hoãn render các thành phần nặng (biểu đồ, trình soạn thảo) đến khi đáp ứng điều kiện cụ thể. Kết hợp tốt với lazy loading và `loadComponent`.
```html
@defer (on viewport; prefetch on hover) {
  <app-heavy-chart [reportId]="reportId()" />
} @placeholder {
  <app-skeleton lines="6" />
} @loading {
  <p>Đang tải biểu đồ...</p>
} @error (let err) {
  <p class="error">Không thể tải biểu đồ: {{ err }}</p>
}
```
- **`ngTemplateOutlet`**: Với các phần giao diện lặp lại nhưng khác dữ liệu, đóng gói nội dung vào `<ng-template>` và tái sử dụng bằng `ngTemplateOutlet`. Điều này giúp bài học component nâng cao rõ ràng và dễ kiểm thử hơn.

## 12. Chia sẻ trạng thái và mô hình DI hiện đại
- **Service + Signal Store**: Khi nhiều component cần chung một nguồn dữ liệu, hãy gom logic vào service và dùng `signal` hoặc `computed` để phơi bày state đã được đóng gói.
```ts
...
export class TodosStore {
  ...
  load() {
    return this.http.get<Todo[]>('/api/todos').subscribe({
      next: todos => this._todos.set(todos)
    });
  }
}
```
- **Truy cập service ngoài component**: Angular 15+ cho phép gọi `inject()` trong bất kỳ hàm thuần nào khi đang ở trong **injection context** (ví dụ constructor, `setup()` hoặc `runInInjectionContext`). Đây là cách tốt để viết helper chia sẻ logic mà không cần tạo class mới.
```ts
import { runInInjectionContext } from '@angular/core';

export function withCurrentUser<T>(fn: (user: User) => T) {
  return runInInjectionContext(injector, () => {
    const auth = inject(AuthStore);
    return fn(auth.currentUser());
  });
}
```
- **Phạm vi provider**: Nếu service chỉ dùng ở một feature, đặt vào `providers` của component/route để Angular tạo instance riêng mỗi lần route được kích hoạt. Điều này giúp giảm memory footprint và làm ví dụ về state cục bộ rõ ràng hơn.
- **Hủy đăng ký tự động**: Từ Angular 16 có `takeUntilDestroyed()` và từ 17 có `DestroyRef`. Áp dụng chúng cho mọi subscription viết trong tài liệu để người đọc thấy cách xử lý leak đúng chuẩn.
