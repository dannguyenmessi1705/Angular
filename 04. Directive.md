...
## 5. Renderer2 và thao tác DOM an toàn
- Tránh truy cập trực tiếp vào `nativeElement` khi bạn muốn thay đổi DOM (đặc biệt trong môi trường SSR hoặc Web Worker). Angular cung cấp `Renderer2` để thao tác DOM an toàn và tương thích đa nền tảng.
```ts
...
export class ElevatedDirective {
  ...
  addShadow() {
    this.renderer.setStyle(this.host.nativeElement, 'box-shadow', '0 8px 24px rgba(0,0,0,0.15)');
  }

  @HostListener('mouseleave')
  removeShadow() {
    this.renderer.removeStyle(this.host.nativeElement, 'box-shadow');
  }
}
```
- Với `Renderer2` bạn cũng có thể thêm class (`addClass`/`removeClass`), tạo phần tử (`createElement`) hoặc lắng nghe sự kiện (`listen`). Điều này giúp tài liệu phù hợp cả khi học viên bật chế độ SSR như đã hướng dẫn trong `11.Configuration.md`.

## 6. Host directives và tái sử dụng logic
- Angular 15+ cho phép component nhập trực tiếp một directive khác thông qua thuộc tính `hostDirectives`. Cách làm này phù hợp khi bạn muốn chia sẻ behavior (ví dụ xử lý focus, tooltip) cho nhiều component mà không cần ghi nhớ thêm thuộc tính trong template.
```ts
@Directive({
  selector: '[appTooltip]',
  standalone: true,
  ...
  hostDirectives: [
    {
      ...
      inputs: ['text: tooltipText'], // alias input => tooltipText trên component
    },
  ],
})
export class IconButtonComponent {}
```
- Bạn cũng có thể export các `output` từ host directive để component cha lắng nghe sự kiện (ví dụ `tooltipShown`). Đây là kỹ thuật rất hữu ích khi xây dựng thư viện design system: directive giữ logic phức tạp, component đảm nhận giao diện.
- Kết hợp host directives với DI scope riêng (thêm `providers` ngay trong directive) sẽ giúp các ví dụ quản lý trạng thái trở nên độc lập và dễ gỡ lỗi hơn.
