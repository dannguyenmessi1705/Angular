...
Trong component, lấy thông tin từ `ActivatedRoute` bằng cách gọi `snapshot` (không cần dùng Observables) vì dữ liệu tĩnh không thay đổi trong suốt vòng đời của component.
```ts
...
export class ExampleComponent {
  ...
  ngOnInit(): void {
    const title = this.activatedRoute.snapshot.data['title']; // Lấy dữ liệu tĩnh từ route bằng `snapshot`
    const description = this.activatedRoute.snapshot.data['description']; // Lấy dữ liệu tĩnh từ route bằng `snapshot`
