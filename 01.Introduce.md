...
## 6. Chu trình phát triển một ứng dụng Angular
- **Chuẩn bị môi trường**: Cài Node.js (khuyến nghị sử dụng `nvm` để chuyển phiên bản), Angular CLI (`npm install -g @angular/cli`) và trình quản lý gói yêu thích (npm, pnpm, yarn). Luôn đảm bảo phiên bản CLI khớp với phiên bản dự án (`ng version`).
- **Khởi tạo & cấu hình**: Dùng `ng new` hoặc `ng g application` để tạo workspace, bật tùy chọn `--routing`, `--ssr`, `--style=scss` nếu cần từ đầu. Kiểm tra lại `tsconfig.json`, `angular.json`, `package.json` sau khi tạo để bật `strictTemplates`, `forceConsistentCasingInFileNames`, `esModuleInterop` và các rule ESLint cần thiết.
- **Phát triển**: Chạy `ng serve --hmr --open` trong quá trình làm việc. Tổ chức mã theo thành phần/mô-đun như gợi ý ở trên, tận dụng `standalone` component để giảm boilerplate và dùng Signals để quản lý trạng thái cục bộ.
- **Kiểm thử**: Dùng `ng test --watch=false` cho unit test, `ng e2e` cho end-to-end. Với dự án mới hãy cấu hình sớm các công cụ như Jest, Playwright, Cypress để đảm bảo tài liệu hướng dẫn đi kèm luôn có ví dụ kiểm thử.
- **Build & kiểm tra hiệu năng**: `ng build --configuration production` sẽ kích hoạt AOT, tối ưu hóa tree-shaking, Terser và CSS minify. Dùng thêm `ng build --stats-json` kết hợp `ng analyze` hoặc `webpack-bundle-analyzer` để tìm module lớn cần lazy load.
- **Triển khai**: Với SPA truyền thống, phục vụ qua CDN hoặc dịch vụ static hosting (Firebase Hosting, Vercel…). Nếu dùng SSR/SSG hãy bật Angular Universal và thiết lập pipeline CI/CD (GitHub Actions, GitLab CI) để tự động hóa `build:test:deploy`.

## 7. Standalone API so với NgModule
- **Standalone component/directive/pipe**: Được giới thiệu từ Angular 15, loại bỏ nhu cầu khai báo trong `@NgModule`. Bạn chỉ cần thêm tài nguyên vào thuộc tính `imports` của component đang dùng. Cách này đơn giản hóa ví dụ trong tài liệu và giảm thời gian tải.
- **NgModule truyền thống**: Vẫn hữu ích cho các dự án lớn cần gom nhóm theo feature, chia sẻ provider cấp module hoặc xuất bản thư viện Angular. Khi dùng module, hãy bật `standalone: false` (mặc định) và khai báo resources trong `declarations`, `imports`, `exports`.
- **Chiến lược hỗn hợp**: Phần lớn dự án hiện nay vận hành ở chế độ “module bootstrap + standalone feature”. Nghĩa là `AppModule` chịu trách nhiệm khởi động (đặc biệt nếu còn dùng thư viện cũ) trong khi các feature mới viết kiểu standalone. Điều này giúp nâng cấp dần mà không phải viết lại toàn bộ.
- **Ảnh hưởng tới test & DI**: Standalone trả lời bằng cách dùng `bootstrapApplication` và `ApplicationConfig` thay cho `AppModule`. Khi viết test, Angular TestBed đã hỗ trợ `imports: [MyStandaloneComponent]`, tuy nhiên nếu dự án cũ của bạn vẫn dùng module hãy giữ cấu trúc rõ ràng để tránh nhầm lẫn.

## 8. Nguồn tham khảo và tài liệu kèm theo
- **Angular DevTools & tài liệu chính thức**: https://angular.dev là nơi cập nhật các tính năng Signals, hydration, zoneless. Khi bổ sung bài học mới, hãy ghi rõ phiên bản Angular mà ví dụ của bạn sử dụng để người đọc dễ kiểm chứng.
- **Tài liệu cấu hình môi trường**: xem thêm `11.Configuration.md` để biết cách thiết lập Node, Angular CLI, per-environment build targets và hướng dẫn cấu hình đa ngôn ngữ (i18n) khi triển khai thực tế. Bạn cũng có thể trích dẫn trực tiếp file này cho học viên mới tải repo về.
- **Các tiện ích nên cài**: Angular Language Service (VS Code), ESLint, Prettier, EditorConfig, Angular DevTools extension. Những công cụ này giúp đảm bảo mọi người trong nhóm làm theo đúng chuẩn đặt ra tại mục “Quy tắc cấu trúc dự án”.
